...
@section('content')
<div class="container">
    <h1>Edit Fakultas: {{ $faculty->name }}</h1>
    <form action="{{ route('faculties.update', $faculty->id) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="form-group mb-3">
            <label for="name">Nama Fakultas</label>
            <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $faculty->name) }}">
            @error('name')<div class="invalid-feedback">{{ $message }}</div>@enderror
        </div>
        <div class="form-group mb-3">
            <label for="faculty_code">Kode Fakultas</label>
            <input type="text" class="form-control @error('faculty_code') is-invalid @enderror" id="faculty_code" name="faculty_code" value="{{ old('faculty_code', $faculty->faculty_code) }}">
            @error('faculty_code')<div class="invalid-feedback">{{ $message }}</div>@enderror
        </div>
        <div class="form-group mb-3">
            <label for="dean">Dekan</label>
            <input type="text" class="form-control @error('dean') is-invalid @enderror" id="dean" name="dean" value="{{ old('dean', $faculty->dean) }}">
        ...
        <div class="form-group mb-3">
            ...
            <select class="form-control @error('university_id') is-invalid @enderror" id="university_id" name="university_id">
                ...
                @foreach ($universities as $university)
                    <option value="{{ $university->id }}" {{ old('university_id', $faculty->university_id) == $university->id ? 'selected' : '' }}>{{ $university->name }}</option>
                @endforeach
            </select>
